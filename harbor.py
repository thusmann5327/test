...
def save_line_items(conn, document_id, line_items_data, items_data):
    ...
    for line_item in line_items_data.get('Value', []):
        ...
        quantity = line_item.get('OrderQuantity', 0)
        unit_price = line_item.get('UnitPrice', 0.0)
        line_total = quantity * unit_price
        
        # Determine if sold by each or otherwise
        sold_by = 'each' if line_item.get('UOM', '').upper() == 'EA' else 'case'
        
        # Cost per unit
        cost_per_unit = unit_price / quantity if quantity > 0 else 0.0
        ...
        (invoice_id, item_id, item_description, brand_name, category_id,
         ...
         margin_pct, package_description, line_total, cost_per_unit, sold_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            document_id,
            item_id,
            item_detail.get('Description', line_item.get('Description', 'N/A')),
            item_detail.get('Brand', line_item.get('Brand', 'N/A')),
            item_detail.get('CategoryID', line_item.get('CategoryID', 'N/A')),
            unit_price,
            line_item.get('ExtCost', 0.0),
            quantity,
            line_item.get('UOM', 'N/A'),
            item_detail.get('UPC', 'N/A'),
            item_detail.get('VendorID', 'N/A'),
            item_detail.get('SRP', line_item.get('SRP', 0.0)),
            item_detail.get('Margin', line_item.get('Margin', 0.0)),
            item_detail.get('Packaging', line_item.get('Packaging', 'N/A')),
            line_total,
            cost_per_unit,
            sold_by
        ))
...
def check_database():
    ...
    try:
        ...
        # Check line items
        cursor.execute("""
        SELECT invoice_id, item_id, item_description, unit_price, srp, quantity, line_total, cost_per_unit, sold_by 
        ...
        for item in items:
            print(f"Invoice {item[0]}: {item[1]} - {item[2]} - ${item[3]:.2f} x {item[5]} (SRP: ${item[4]:.2f}, Cost/Unit: ${item[7]:.2f}, Sold By: {item[8]}) = ${item[6]:.2f}")
